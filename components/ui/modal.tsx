      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className="relative w-full max-w-lg rounded-2xl border border-zinc-700/50 bg-zinc-900/95 shadow-2xl shadow-black/60 backdrop-blur-xl focus-visible:outline-none overflow-hidden"
          >
            {/* Top accent bar */}
            <div className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-emerald-500/60 to-transparent" />

            {/* Decorative glows */}
            <div className="absolute -top-24 -left-24 w-48 h-48 bg-emerald-500/8 rounded-full blur-3xl pointer-events-none" />
            <div className="absolute -bottom-24 -right-24 w-48 h-48 bg-teal-500/8 rounded-full blur-3xl pointer-events-none" />

            {/* Header */}
            <div className="relative flex items-start justify-between p-6 pb-4 border-b border-zinc-800/60">
              <div>
              ...
              <button
                ...
                className="p-1.5 rounded-full text-zinc-500 bg-zinc-800/50 hover:text-white hover:bg-zinc-700 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            {/* Body */}
            <div className="relative p-6 text-zinc-300">{children}</div>
          </motion.div>
        </motion.div>
      )}
    </AnimatePresence>,
  ...
  if (confirmed) {
    return (
      <div className="flex flex-col items-center gap-4 py-4 text-center">
        ...
        <p className="text-white font-semibold text-lg">Agendamento confirmado!</p>
  ...
  return (
    <div className="space-y-5">
      <p className="text-sm text-zinc-400 leading-relaxed">
        ...
        <strong className="text-emerald-400">projeto demonstrativo para fins de estudo</strong>. Selecione um horário fictício abaixo ou conheça o desenvolvedor.
      </p>
      ...
      <div className="space-y-3">
        ...
        {DEMO_SLOTS.map((group) => (
          <div key={group.day}>
            <p className="text-xs text-zinc-600 mb-1.5 font-medium">{group.day}</p>
              ...
              {group.times.map((time) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 cursor-pointer',
                      isActive
                        ? 'bg-emerald-500/15 border-emerald-500/50 text-emerald-300'
                        : 'bg-zinc-800/50 border-zinc-700/60 text-zinc-400 hover:border-zinc-600 hover:text-zinc-200'
                    )}
                  >
      ...
      <Button
        ...
        className="w-full gap-2 bg-emerald-600 hover:bg-emerald-700 focus-visible:ring-emerald-500"
      >
      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-950/60 p-4 space-y-3">
        ...
        <div className="flex flex-col gap-2">
          ...
          >
            <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" className="text-zinc-500 group-hover:text-white transition-colors" aria-hidden="true"><path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" /><path d="M9 18c-4.51 2-5-2-7-2" /></svg>
